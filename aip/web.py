    ...
    def handle_request(environ, start_response):
        ...
        try:
            ...
            if method == "POST" and len(parts) == 3 and parts[0] == "sessions" and parts[2] == "athletes":
                ...
                return redirect(start_response, f"/sessions/{session_id}")
            if method == "POST" and len(parts) == 4 and parts[0] == "sessions" and parts[2:] == ["athletes", "existing"]:
                session_id = resource_id(parts[1], "session")
                database.add_existing_session_athlete(
                    session_id, resource_id(form_data(environ).get("athlete_id"), "athlete"),
                )
                return redirect(start_response, f"/sessions/{session_id}")
...
def session_page(db: Database, session_id: int) -> str:
    ...
    if group and not completed:
        roster_ids = {athlete["id"] for athlete in athletes}
        existing_options = "".join(
            f"<option value='{athlete['id']}'>{html.escape(athlete['name'])}</option>"
            for athlete in db.all_athletes() if athlete["id"] not in roster_ids
        )
        existing_control = (
            f"""<form method='post' action='/sessions/{session_id}/athletes/existing' class='inline-form'>
              <label>Existing athlete<select name='athlete_id' required><option value=''>Choose athlete</option>{existing_options}</select></label>
              <button>Add existing athlete</button>
            </form><p class='muted'>Keeps the athlete's existing sprint history and profile.</p>"""
            if existing_options else "<p class='muted'>Every existing athlete is already in this session.</p>"
        )
        add_athlete = f"""<details class='add-session-athlete'><summary>Add athlete to active session</summary>
          <div class='late-athlete-options'>
            <details open><summary>Add an existing athlete</summary>{existing_control}</details>
            <details><summary>Create a new profile</summary>
              <form method='post' action='/sessions/{session_id}/athletes' class='inline-form'>
                <label>New athlete name<input name='name' maxlength='100' required placeholder='Athlete name'></label>
                <button>Create profile and add</button>
              </form><p class='notice'>Only use this when the athlete does not already have an AIP profile.</p>
            </details>
          </div><p class='muted'>The athlete will join this session and future {html.escape(group['name'])} sessions.</p>
        </details>"""
    ...
    <main class='capture-layout'>
      ...
      <section class='capture-card'>
        {status_notice}{empty}<form id='capture-form' data-session='{session_id}' data-completed='{'true' if completed else 'false'}'>
          ...
          <div id='active-rep-context' class='active-rep-context'><p class='muted'>Record a time to add rep context.</p></div>
...
function render(data){
  ...
  const rows=data.attempts.map(a=>`<li class="attempt-block ${a.just_saved?'saved':''}"><div class="attempt"><div><strong>${escapeHtml(a.time)}s</strong> ${a.status==='baseline'?'<span class="badge baseline">Baseline</span>':a.status==='pr'?'<span class="badge pr">PR</span>':''} ${a.is_reference?'<span class="badge reference">Reference</span>':''} ${a.effort_instruction||a.coach_observation||a.athlete_feedback||a.video_reference?'<span class="badge reference">Context saved</span>':''}</div><div class="actions"><button type="button" onclick="showRepContext(${a.id})">Rep context</button>${data.editable?`<button type="button" onclick="editAttempt(${a.id}, '${escapeHtml(a.time)}')">Edit</button><button type="button" class="danger" onclick="deleteAttempt(${a.id})">Delete</button>`:''}</div></div></li>`).join('');
  const justSaved=data.attempts.find(a=>a.just_saved);
  const contextAttempt=data.attempts.find(a=>a.id===selectedContextAttemptId)||justSaved||data.attempts[0];
  if(justSaved)selectedContextAttemptId=justSaved.id;
  activeRepContext.innerHTML=contextAttempt?repContextEditor(contextAttempt,Boolean(justSaved)): '<p class="muted">Record a time to add rep context.</p>';
...
async function setAthlete(index,focusTime=true){if(!athletes.length)return;activeIndex=(index+athletes.length)%athletes.length;selectedContextAttemptId=null;const selected=athletes[activeIndex];athlete.value=selected.id;athleteName.textContent=selected.name;athletePosition.textContent=`Athlete ${activeIndex+1} of ${athletes.length}`;athleteSearch.value='';localStorage.setItem(`aip-session-${sessionId}-athlete`,selected.id);renderUpNext();await loadAthlete();if(focusTime&&!completed)elapsed.focus();}
